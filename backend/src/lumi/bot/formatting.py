...
def format_today(payload: dict, tz: str) -> str:
    ...
    if summary["emails_need_reply"]:
        n = summary["emails_need_reply"]
        bits.append(f"{n} {ru_plural(n, 'письмо ждет', 'письма ждут', 'писем ждут')} ответа")
...
HELP_TEXT = """Я Lumi — твой личный ассистент.

Просто пиши мне обычным языком:
• «Напомни завтра в 10 написать Саше»
• «Сделай план на сегодня с учетом встреч»
• «Разбери почту за утро»
• «Каждый будний день в 8:30 присылай новости по AI»
• «Запомни: дайджесты лучше до 9:30»
...
/email — разобрать почту
/app — открыть Mini App
/settings — настройки и статус

Рискованные действия (внешний календарь, почта) я делаю только после твоего подтверждения."""
