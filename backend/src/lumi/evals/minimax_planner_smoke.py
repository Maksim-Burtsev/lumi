...
CASES: tuple[PlannerSmokeCase, ...] = (
    PlannerSmokeCase(
        ...
        expected_args={"project": "Home"},
    ),
    PlannerSmokeCase(
        id="task_project_followup_en",
        message="Also add call the HVAC company to the same project.",
        expected_mode="tool_calls",
        expected_tool="create_task",
        expected_args={"project_ref": "last_task_project"},
        known_context="Recent task action: created task in project Home. last_task_project=Home.",
    ),
    PlannerSmokeCase(
        id="task_bulk_update_en",
        message="Move all onboarding tasks from Work to People Ops.",
        expected_mode="tool_calls",
        expected_tool="bulk_update_tasks",
        expected_args={"from_project": "Work"},
    ),
    PlannerSmokeCase(
