...
async def test_run_morning_brief_sends_rich_schedule_section(monkeypatch, user):
    sent: list[dict] = []
    await _set_locale(user, "ru")
    start = local_to_utc(datetime(2026, 6, 24, 13, 0), user.timezone)
    ...
    async def fake_generate_digest(self, user_arg, *, agent_run_id=None):
        return None
    ...
    monkeypatch.setattr("lumi.services.today.TodayService.build_payload", fake_build_payload)
    monkeypatch.setattr("lumi.connectors.google.auth.token_file_exists", lambda: False)
    monkeypatch.setattr("lumi.services.news.NewsService.generate_digest", fake_generate_digest)
    monkeypatch.setattr("lumi.services.notifier.send_telegram_message", fake_send_telegram_message)

    result = await jobs.run_morning_brief({}, str(user.id), notify=True)
