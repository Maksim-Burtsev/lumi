...
| A standalone browser session is stolen or forged | `/web` issues a five-minute, single-use Redis nonce in the URL fragment. Exchange creates a fixed-expiry Redis session behind a host-only `HttpOnly; Secure; SameSite=Strict` cookie signed with the separate `WEB_SESSION_SECRET`. Unsafe cookie-authenticated requests require the exact `APP_PUBLIC_URL` origin and a derived CSRF token. Logout deletes the server-side session. |
...
| Public tunnel exposure | Only `/app` (static) and `/api` (Telegram initData or server-side web-session auth) are exposed; Postgres/Redis bind to 127.0.0.1; `/docs` and debug are only for `APP_ENV=local`; dev-auth is disabled by default. |
...
`WEB_SESSION_SECRET` is deliberately separate from `APP_SECRET_KEY`. Standalone
login stays disabled unless it is set and `APP_PUBLIC_URL` is a canonical HTTPS
origin. Login nonces, session ids, and CSRF values must never be logged.
